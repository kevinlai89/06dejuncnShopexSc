...
class mdl_certificate extends modelFactory {
    ...
    function getent_ac(){
     ...
     function getent_sign(){
        if($this->system->getConf('enterprise.ent_sign')){
      ...
      function getent_email(){
        if($this->system->getConf('enterprise.ent_email')){
   ...
   function ent_to_shopex_enterprise($certi_app,$format='json',$ent_data){
      $post_entlogin['certi_app']=$certi_app;
           ...
           $url='http://passport.shopex.cn/api.php';//内网shopexuser.ex-sandbox.com
            $net = $this->system->loadModel('utility/http_client');
            $results_entlogin=$net->post($url,$post_entlogin);
            $results_entlogin_json=json_decode($results_entlogin,1);
           if($results_entlogin_json['res']=='succ'){
             return true;
           }else{
               return false;
           }
   ...
   function setent_sign($ent_sign){
    ...
    function getUrl($str,$unlogin=0){
        ...
        $url = 'http://service.shopex.cn/info.php'.$url.'&_key_=do&store_refer=yikaidian';
        
        if ($unlogin)
            $url.="&state=no&store_refer=yikaidian";
