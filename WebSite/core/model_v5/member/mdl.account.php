...
class mdl_account extends shopObject {
    ...
    function create($data,&$message){
        ...
        /*if(!preg_match('/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/', $data['email'])){*/
			if(!preg_match('/^([a-zA-Z0-9]+[_|\-\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_\-|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/', $data['email'])){
            //trigger_error(__('Email输入不正确'), E_USER_ERROR);
            $message=__('Email输入不正确');
            return false;
        }
        ...
        if($this->db->exec($sql)){
            ...
            $this->init($userId);
            $sql = 'select member_id,member_lv_id,email,uname,password,unreadmsg,cur,lang,point from sdb_members where member_id='.$this->db->quote($userId);
    ...
    function cookieValue($memberID){
        $row=$this->db->selectrow('select uname,password from sdb_members where member_id='.$this->db->quote($memberID));
    ...
    */
	 function verifyLogin($login,$passwd,&$message,$passport=null){
        $login = trim(strtolower($login));
        $row = $this->db->selectrow('select * from sdb_members where uname='.$this->db->quote($login)." AND disabled='false'");
        $pwd = $row['password'];

        if ( !$row ) return;
        if ( $pwd != $this->encrypt_passwd_enhanced($passwd,$login,$row['regtime'])  && $pwd != md5($passwd) ) return;

        $this->db->exec('UPDATE sdb_members SET login_count=IFNULL(login_count,0)+1 WHERE member_id='.intval($row['member_id']));
	...
	// 增强的加密算法 加密登录密码
    function encrypt_passwd_enhanced($pwd, $uname, $regtime) {
    ...
    function createUserFromPluin($data,&$message,$uid,$email=''){
        ...
        if(empty($data['email'])||!preg_match('/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/', $data['email'])){
            //trigger_error(__('Email输入不正确'), E_USER_ERROR);
            $message=__('Email输入不正确');
            return false;
        }
    ...
    function createotherlogin($row){
        ...
        if($username){
          if($data = $this->db->selectrow("SELECT mm.member_id,mm.member_lv_id,mm.cur,mm.lang,mm.disabled,ol.uname FROM sdb_members mm LEFT JOIN sdb_trust_login ol ON mm.member_id = ol.member_id WHERE ol.member_id =".$this->db->quote($username['member_id']))){
            ...
            $user['lang']="123";
            $user['email'] = $row['email']?$row['email']:'x@x.com';
    ...
    function adminUpdateMemberPassword($nMId,$aData,$sendemail){
		...
		$aData['password'] = md5($aData['passwd']);
