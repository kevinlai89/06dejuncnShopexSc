...
class mdl_member extends shopObject {
    ...
    function trust_check($trust){
        ...
        $trust_uname = $this->db->selectrow("SELECT show_uname FROM sdb_trust_login WHERE member_id =".$this->db->quote($mem_uname['member_id']));
    ...
    function getMemberInfo($nMemberId){
        ...
        $mem_uname = $this->db->selectrow("SELECT show_uname FROM sdb_trust_login WHERE member_id =".$this->db->quote($nMemberId));
...
/*        if($aInfo['email'] == $aData['email']){
    ...
    function saveMemAttr($data){
        $selsql = "select attr_type from sdb_member_attr where attr_id = ".$this->db->quote($data['attr_id'])."";
    ...
    function getLevelByPoint($nPoint) {
        $sSql = 'SELECT member_lv_id, name FROM sdb_member_lv WHERE point <= '.$this->db->quote($nPoint).' AND lv_type=\'retail\' AND disabled="false" ORDER BY point DESC';
        return $this->db->selectrow($sSql);
    }
    function getLevelByExperience($nExpierence) {
        $sSql = 'SELECT member_lv_id, name FROM sdb_member_lv WHERE experience <= '.$this->db->quote($nExpierence).' AND lv_type=\'retail\' AND disabled="false" ORDER BY experience DESC';
    ...
    function updateMemAttr($member_id,$attr_id,$data){
        $selsql = "select attr_type from sdb_member_attr where attr_id = ".$this->db->quote($attr_id)."";
        $tmpdate = $this->db->select($selsql);
