...
class ctl_product extends shopPage{
    ...
    function index($gid,$specImg='',$spec_id='') {
        ...
        if(!$aGoods = $objGoods->getGoods($gid,$member_lv)){
          ...
          $bylimit=	$aGoods['bylimit'];
					$order_single_min=$aGoods['order_single_min'];
					$order_day_min=$aGoods['order_day_min'];
					$order_all_min=$aGoods['order_all_min'];
					$order_single_max=$aGoods['order_single_max'];
					$order_day_max=$aGoods['order_day_max'];
					$order_all_max=$aGoods['order_all_max'];      
          $this->pagedata['bylimitinfo']="";
          $this->pagedata['defaultnums_min']=1;//low
          $this->pagedata['defaultnums_max']=0;//max
                ...
                if ($commentList['total']){
                   ...
                   foreach($aComment['list'][$item] as $key => $rows){
                     foreach($aReply as $rkey => $rrows){
                      ...
                      $aComment['list'][$item][$key]['goods_point']=$commentpointone?$commentpointone:"5";//if no star,set 5 stars
        ...
        /**** end 商品评论 ****/
// *** begin 评分类型 by showker-2011-6-16 ****
       ...
       // print_r( $commentRows);exit;
        ...
        $this->pagedata['point_peoples']=$objPoint->point_peoples($gid);
        /**** end 评分类型 by showker-2011-6-16 ****/
#####################################
        /**** begin 相关商品 ****/
        ...
        if(count($aLinkId['goods_id'])>0){
            ...
            /*add stars 7-7-begin*/
            foreach($this->pagedata['goods']['link'] as $k=>$v){
              $this->pagedata['goods']['link'][$k]['goods_point']= $objPoint->get_single_point($v['goods_id']);
            }
            /*add stars 7-7-end*/
