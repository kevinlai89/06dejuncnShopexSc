...
class mdl_countdownlog extends shopObject{
		var $idColumn = "cat_id";
		var $textColumn = "cat";
		...
		function save( $aData ){
			...
			foreach($goodsArray as $goods_id){
				...
				$limit_num = $aData['limit_num'][$goods_id];
		...
		{
			$sql = "SELECT * FROM sdb_countdown_cat WHERE cat_id=".$catId;
			return $this->db->selectRow( $sql );
		}


		public function getCountdownGoods( $catId )
		{
			$sSql = "SELECT c.*,if(c.limit_end_time<unix_timestamp(now()), 0, c.limit_end_time-unix_timestamp(now())) as lefttime,g.marketable,g.disabled,g.thumbnail_pic,g.small_pic,g.big_pic,g.intro,g.mktprice,g.price,g.name,g.marketable,g.store FROM sdb_countdown_list c\n LEFT JOIN sdb_goods g ON c.goods_id = g.goods_id\n WHERE c.cat_id = ".intval( $catId );
