...
class mdl_taobao extends app_taobao_goods{
    ...
    function __construct(){
        ...
        if(!defined('SAAS_API_URL')){
            define('SAAS_API_URL','http://api-test.dianzhanggui.net/api.php');
        }
    ...
    function get_postages($args){
        ...
        $ss = array($rsp['postages_get_response']['postages']['postage']);
        return $ss;
    }
    ...
    function taobao_item_add($data, &$errMsg){
        $params = $this->api[__FUNCTION__];
        if($data['iid'] || $data['num_iid']) $params = $this->api['taobao_item_update'];
        ...
        if($rsp['error_response']){
            ...
            if($data['iid'] || $data['num_iid']) return $rsp['item_update_response'][0]['num_iid'];
            return $rsp['item_add_response'][0]['num_iid'];
        }
    }
