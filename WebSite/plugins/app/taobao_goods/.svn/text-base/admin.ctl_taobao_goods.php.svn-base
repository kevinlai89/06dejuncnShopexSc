...
class admin_ctl_taobao_goods extends pageFactory{
    ...
    function taobao_item_add($goods_id){
        ...
        if($row['disabled']=='false'){
            ...
            $data['content']['price'] = number_format($data['content']['price'],2,'.','');
            //如果是编辑
            if($row['outer_id']){
                if(is_numeric($row['outer_id']))
                    $data['content']['num_iid'] = $row['outer_id'];
                else
    ...
    function getTbloginurl($url){
        ...
        if($return){
            $tbs_params['app_key'] = $return['result_msg']['app_key'];
            $tbs_params['app_secret'] = $return['result_msg']['app_secret'];
        }
        date_default_timezone_set('PRC');
        $tbs_params['sign_method'] = "md5";
        $tbs_params['timestamp'] = date('Y-m-d H:i:s',time());
        $tbs_params['target'] = substr(($url?$url:$this->system->base_url().'shopadmin/index.php?ctl=plugins/ctl_taobao_goods'),7);
