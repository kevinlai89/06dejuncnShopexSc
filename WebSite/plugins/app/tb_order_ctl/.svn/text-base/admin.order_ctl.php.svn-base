...
class admin_order_ctl extends ctl_order{
    ...
    function detail_info($order_id){
        ...
        foreach($this->pagedata['goodsItems'] as $k=>$v){
            ...
            if(isset($this->pagedata['goodsItems'][$k]['taobao_iid']) && is_numeric($this->pagedata['goodsItems'][$k]['taobao_iid'])){
                $this->pagedata['goodsItems'][$k]['key_name'] = "item_num_id";
            }
            else{
                $this->pagedata['goodsItems'][$k]['key_name'] = "item_id";
                $taobao_iids[] = $this->pagedata['goodsItems'][$k]['taobao_iid'];
            }
        ...
        if(count($taobao_iids)>0){
            $params['fields'] = "iid,num_iid,outer_id";
            $params['method'] = "taobao.items.list.get";
            $params['iids'] = implode(",",$taobao_iids);
            $return = $this->tb->getContents($params);
            if($return['items_list_get_response']['items'] && count($return['items_list_get_response']['items'])>0){
                foreach($return['items_list_get_response']['items'] as $k2=>$v2){
                    $rs = $this->db->exec("select * from sdb_tb_order_ctl_order_items where taobao_iid='".$v2['iid']."'");
                    $ssql=$this->db->getUpdateSQL($rs,array('taobao_outer_id'=>$v2['outer_id'],'taobao_iid'=>$v2['num_iid']));
                    $this->db->exec($ssql);
                }
            }
            unset($params);
    ...
    function set_sync_time(){
        $this->display("view/set_sync_time.html");
    }
    function save_sync_time(){
        $status = $this->system->loadModel('system/status');
        $status->set('LAST_SYNCHRONIZATION',$_POST['sync_time']);
        $this->index();
    }
