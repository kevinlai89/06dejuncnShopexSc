...
//review 读setting表
    ...
    function output(){
        ...
        foreach(unserialize($this->system->getConf('set.app.list')) as $key=>$ident){
            $app = $appmgr->load($ident);
            if(method_exists($app, 'modify_pagedata')){
                $app->modify_pagedata($this);
            }
        }
            ...
            foreach($trust_tag as $tt =>$tv){
                foreach($openid_logo as $k =>$v){
                     ...
                     if($tt == $open){
                        ...
                        $openid_logo[$trust_tag[$open]]['href'] = TRUST_LOGIN_URL.'?open='.$open.'&certi_id='.$this->system->getConf('certificate.id').'&callback_url='.$this->system->base_url().'&local_type=direct&callback='.$_SERVER['HTTP_REFERER'];
