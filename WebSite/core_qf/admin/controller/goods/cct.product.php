...
class cct_product extends objectPage{
    ...
    function addNew($cat_id=null,$gid=null,$type_id=null,$brand_id=null){
	         /*sk*/
        $tuangou_cat = &$this->system->loadModel('tuan/tuangou_cat');   
        $this->pagedata['tuancats']=$tuangou_cat->get_cat_list();
        /*sk*/
        ...
        $objGoods = $this->system->loadModel('trading/goods');
        $this->pagedata['qtn_template_list'] = $objGoods->getQtnTemplateList();

        $objProducts = $this->system->loadModel('goods/products');
        $cols = $objProducts->getColumns();
        $this->pagedata['qtn_show_types'] = $cols['qtn_show']['type'];
    ...
    function edit($goods_id){
        ...
        /*add show tuaninfo begin showker*/
        $dbstuan=$this->system->database();
        $sqltuaninfo="select * from sdb_tuangou_team where goods_id=".$goods_id." order by id desc";//." and begin_time=".$goods['uptime'];  
        $tuaninfo=$dbstuan->selectRow($sqltuaninfo); 
        if(is_array($tuaninfo) && count($tuaninfo)>0){
        $tuaninfo['intro']=str_replace('\r','<br>',stripslashes($tuaninfo['intro']));
        $this->pagedata['tuan']=$tuaninfo;
        $this->pagedata['tuan']['begin_time_h']=date('H',$tuaninfo['begin_time']);
        $this->pagedata['tuan']['begin_time_m']=date('i',$tuaninfo['begin_time']);
        $this->pagedata['tuan']['expire_time_h']=date('H',$tuaninfo['expire_time']);
        $this->pagedata['tuan']['expire_time_m']=date('i',$tuaninfo['expire_time']);  
        }
          /*sk*/
        $tuangou_cat = &$this->system->loadModel('tuan/tuangou_cat');   
        $this->pagedata['tuancats']=$tuangou_cat->get_cat_list();
        /*sk*/
       //print_r($this->pagedata['tuancats']);exit;     
        /*add show tuaninfo end showker*/
        if($goods['products']){
        ...
        $goods['tag_pic'] = unserialize($goods['tag_pic']);
    ...
    function _editor($type_id){
        ...
        $tag_status = $this->system->getConf('site.tag_status');
        $tag_rule = array();
        ...
        $this->pagedata['tag_status'] = $tag_status;
    ...
    function toAdd(){
        $data = $_POST['goods'];
        $data['tag_pic'] = serialize($_POST['tag']);
...
//       $data['spec_desc'] = addslashes_array($data['spec_desc']);
        ...
        /*add tuan do begin by showker*/
        if($data['istuan']=="true"){
     
          
          $goodsinfo = $objGoods->getGoods($gid);
          $datatuan=$_POST['tuan'];//post tuan array 2011-3-31
          if($datatuan['intro']=='&nbsp;' || $datatuan['intro']=='<br/>'){
            $datatuan['intro']='';
          }
           /*suolvtu*/
          //$objtuan = &$this->system->loadModel('tuan/tuangou_team');
          //$datatuan['tuan_big_pic']=$_POST['tuan_big_pic'];//$objtuan->savetuanpic($_POST);
          if(!$datatuan['tuan_big_pic']){unset($datatuan['tuan_big_pic']);}
          /*suolvtu*/
            $dbstuan=&$this->system->database();
            //$sqltuaninfo="select id from sktuangou_team where goods_id=".$gid." and begin_time=".$goodsinfo['uptime'];  
            $sqltuaninfo="select id from sdb_tuangou_team where goods_id=".$gid." and begin_time=".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']);//gid plus begin_time if is one team
            $tuaninfo=$dbstuan->select($sqltuaninfo);            
            $datatuan['max_number']=$datatuan['max_number']?$datatuan['max_number']:($data['store']?$data['store']:0);//tuangou top limit
            $datatuan['price']=$datatuan['price']?$datatuan['price']:($data['price']?$data['price']:0);//tuangou price
            $datatuan['mktprice']=$datatuan['mktprice']?$datatuan['mktprice']:($data['mktprice']?$data['mktprice']:0);//tuangou mktprice
            $tuanlimit=$data['minorder']?$data['minorder']:0;
            if(count($tuaninfo)==0)
            {
            //不存在此同goods_id,上架时间时增加记录
              $datatuan['now_number']=0;
              $datatuan['state']='none';
              $sqltuan = "insert into sdb_tuangou_team(goods_id,per_number,min_number,max_number,now_number,pre_number,state,conduser,buyonce,expire_time,begin_time,price,mktprice,name,thumbnail_pic,small_pic,big_pic,brief,intro,cat_id) values(".$gid.",".$tuanlimit.",".$datatuan['min_number'].",".$datatuan['max_number'].",".$datatuan['now_number'].",".$datatuan['pre_number'].",'".$datatuan['state']."','".$datatuan['conduser']."','".$datatuan['buyonce']."',".strtotime($datatuan['expire_time']." ".$datatuan['expire_time_h'].":".$datatuan['expire_time_m']).",".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']).",".$datatuan['price'].",".$datatuan['mktprice'].",'".($datatuan['name']?$datatuan['name']:$data['name'])."','".($goodsinfo['thumbnail_pic'])."','".($goodsinfo['small_pic'])."','".($datatuan['tuan_big_pic']?$datatuan['tuan_big_pic']:$goodsinfo['big_pic'])."',".$dbstuan->quote($datatuan['brief']?$datatuan['brief']:$data['brief']).",".$dbstuan->quote($datatuan['intro']?$datatuan['intro']:$data['intro']).",'".$datatuan['cat_id']."')";
              $dbstuan->exec($sqltuan);                  
            }
            else
            {
            //存在则修改
            if($datatuan['tuan_big_pic'])
              {
              $sqltuan="update sdb_tuangou_team set  per_number=".$tuanlimit.",min_number=".$datatuan['min_number'].",max_number=".$datatuan['max_number'].",pre_number=".$datatuan['pre_number'].",conduser='".$datatuan['conduser']."',buyonce='".$datatuan['buyonce']."',expire_time=".strtotime($datatuan['expire_time']." ".$datatuan['expire_time_h'].":".$datatuan['expire_time_m']).",begin_time=".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']).",price=".$datatuan['price'].",mktprice=".$datatuan['mktprice'].",name='".($datatuan['name']?$datatuan['name']:$data['name'])."',thumbnail_pic='".($goodsinfo['thumbnail_pic'])."',small_pic='".($goodsinfo['small_pic'])."',big_pic='".($datatuan['tuan_big_pic'])."',brief=".$dbstuan->quote($datatuan['brief']?$datatuan['brief']:$data['brief']).",intro=".$dbstuan->quote($datatuan['intro']?$datatuan['intro']:$data['intro']).",cat_id='".$datatuan['cat_id']."' where goods_id=".$gid." and begin_time=".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']);//." still add begin_time
             // print_r( $sqltuan);exit;
              }
              else{
                  $sqltuan="update sdb_tuangou_team set  per_number=".$tuanlimit.",min_number=".$datatuan['min_number'].",max_number=".$datatuan['max_number'].",pre_number=".$datatuan['pre_number'].",conduser='".$datatuan['conduser']."',buyonce='".$datatuan['buyonce']."',expire_time=".strtotime($datatuan['expire_time']." ".$datatuan['expire_time_h'].":".$datatuan['expire_time_m']).",begin_time=".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']).",price=".$datatuan['price'].",mktprice=".$datatuan['mktprice'].",name='".($datatuan['name']?$datatuan['name']:$data['name'])."',thumbnail_pic='".($goodsinfo['thumbnail_pic'])."',small_pic='".($goodsinfo['small_pic'])."',brief=".$dbstuan->quote($datatuan['brief']?$datatuan['brief']:$data['brief']).",intro=".$dbstuan->quote($datatuan['intro']?$datatuan['intro']:$data['intro']).",cat_id=".$datatuan['cat_id']." where goods_id=".$gid." and begin_time=".strtotime($datatuan['begin_time']." ".$datatuan['begin_time_h'].":".$datatuan['begin_time_m']);//." still add 
                  //print_r('fasdfasdf'.$sqltuan);exit;
                 // print_r( $sqltuan);exit;
              }
              //print_r($sqltuan);exit;
              $dbstuan->exec($sqltuan);
        ...
        //delete this tuan_team
        else{
          //$sqltuan="delete from sdb_tuangou_team where "
        }
        
        
        /*add tuan do end by showker*/
...
//                    }
  ...
  function qtnconfig(){
      $objGoods = $this->system->loadModel('trading/goods');
      $this->pagedata['config'] = $objGoods->get_qtn_config();
      $this->page('product/qtnconfig.html');
    }

    function saveqtnconfig(){
      //echo '<PRE>';var_dump($_POST);      
      $this->begin('index.php?ctl=goods/product&act=qtnconfig');
      $objGoods = $this->system->loadModel('trading/goods');
      $objGoods->save_qtn_config($_POST);
      $this->system->cache->clear();
      $this->end(true);
    }

    function qtntemplate(){
      $objGoods = $this->system->loadModel('trading/goods');
      $this->pagedata['template_list'] = $objGoods->getQtnTemplateList();
      $this->page('product/qtntemplate.html');
    }

    function editQtntemplate($id){
        $objGoods = $this->system->loadModel('trading/goods');
        $template = $objGoods->getQtnTemplate($id);
        if (!$template){
            $this->begin('index.php?ctl=goods/product&act=qtntemplate');
            $this->end(false,'未找到询价模板!');
            return;
        }
        while (count($template['cus_items']) < 8){
            $template['cus_items'][] = array('name' => '', 'required' => false);
        }
        $this->pagedata['template'] = $template;
        $this->pagedata['cur_action'] = 'edit';
        $this->page('product/qtntemplate_edit.html');
    }

    function addQtntemplate(){
        $objGoods = $this->system->loadModel('trading/goods');
        $this->pagedata['cur_action'] = 'add';
        $this->pagedata['template'] = $objGoods->getBlankQtnTemplate();        
        $this->page('product/qtntemplate_edit.html');
    }

    function saveQtntemplate(){
      //echo '<PRE>';var_dump($_POST);exit;
      $this->begin('index.php?ctl=goods/product&act=qtntemplate');
      $objGoods = $this->system->loadModel('trading/goods');
      $objGoods->saveQtnTemplate($_POST);
      $this->end(true);
    }
