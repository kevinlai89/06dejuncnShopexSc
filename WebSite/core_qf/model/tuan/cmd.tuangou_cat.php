...
class cmd_tuangou_cat extends shopObject{
    ...
    function getAll($catid=null){
        ...
        return $this->db->select('SELECT cat_name AS text, cat_id AS id, is_leaf AS cls FROM sdb_tuangou_cat
            WHERE parent_id '.$catid.' ORDER BY cat_id,p_order desc');
    }
    function getExists(){
        return $this->db->select('select cat_id from sdb_tuangou_cat');
    }

    function getPath($cat_id,$method=null){
        $method = $this->system->getConf('gallery.default_view');
        $row = $this->db->selectrow('select cat_path,cat_name from sdb_tuangou_cat where cat_id='.intval($cat_id));
        ...
        if($row['cat_path'] != ',' && $row['cat_path']){
            ...
            foreach($this->db->select('select cat_name,cat_id from sdb_tuangou_cat where cat_id in('.substr($row['cat_path'],0,-1).') order by cat_path desc') as $row){
    ...
    function get_cat_depth(){
        $row = $this->db->selectrow('select cat_path from sdb_tuangou_cat order by cat_path desc');
        return count(explode(',',$row['cat_path']));
    }
    function get_cat_list($show_stable=false){
        $file=MEDIA_DIR.'/goods_brandcat.data';
    ...
    function getMap($depth=-1,$cat_id=0){
        ...
        if($cat_id>0){
            $row = $this->db->select('select cat_path from sdb_tuangou_cat where cat_id='.intval($cat_id));
            if($depth>0){
                $depth += substr_count($row['cat_path'],',');
            }
            $rows = $this->db->select('select cat_name,cat_id,parent_id,is_leaf,cat_path,type_id from sdb_tuangou_cat where cat_path like "'.$row['cat_path'].$cat_id.'%" order by cat_path,p_order');
        }else{
            $rows = $this->db->select('select cat_name,cat_id,parent_id,is_leaf,cat_path,type_id from sdb_tuangou_cat order by p_order');
        }
        $cats = array();
        $ret = array();
    ...
    function setTabs($catid,$tabs){
        $rs = $this->db->exec('select tabs,cat_id from sdb_tuangou_cat where cat_id='.intval($catid));
        if($rs){
    ...
    function getTabs($catid){
        $row = $this->db->selectrow('select tabs,cat_id from sdb_tuangou_cat where cat_id='.intval($catid));
    ...
    function getNodes($catid=null){
        ...
        return $this->db->select('SELECT p.cat_name as text,p.cat_id as id,c.cat_id as has_child FROM sdb_tuangou_cat p left join sdb_tuangou_cat c on c.parent_id=p.cat_id'.$sqlWhere.' group by(p.cat_id) order by p.p_order, p.cat_id');
    }

    function getCat($catid=0){
        $sqlWhere = ' WHERE parent_id = '.intval($catid);
        return $this->db->select('SELECT * FROM sdb_tuangou_cat'.$sqlWhere);
    }

    function updateOrder($p_order){
        foreach($p_order as $k=>$v){
            $this->db->exec('update sdb_tuangou_cat set p_order='.intval($v).' where cat_id='.intval($k));
        }
        $this->cat2json();
        return true;
    }
    ...
    function getTree(){
        return $this->db->select('SELECT o.cat_name AS text,o.cat_id AS id,o.parent_id AS pid,o.p_order,o.cat_path,
                    is_leaf,o.type_id as type,o.child_count,t.name as type_name FROM sdb_tuangou_cat o
    ...
    function getTreeList($pid=0, $listMark='all'){
        ...
        if($listMark == 'all'){
            $aCat = $this->db->select('SELECT cat_name,cat_id,o.parent_id AS pid,o.p_order,o.cat_path,o.is_leaf AS cls,o.type_id as type
                    FROM sdb_tuangou_cat o WHERE o.disabled=\'false\' ORDER BY o.cat_path,o.p_order,o.cat_id');
        }else{
              if($pid === 0){
            ...
            $aCat = $this->db->select('SELECT cat_name, cat_id, o.parent_id AS pid, o.p_order, o.cat_path, o.is_leaf AS cls,o.type_id, t.name AS type_name FROM sdb_tuangou_cat o
    ...
    function checkTreeSize(){
        $aCount = $this->db->selectrow('SELECT count(*) AS rowNum FROM sdb_tuangou_cat');
    ...
    function getCatParentById($id,$view='index'){
        if(!$id) return false;
            if(is_array($id)){
                ...
                $sqlString = 'SELECT cat_id,cat_name FROM sdb_tuangou_cat WHERE parent_id in ('.implode($id,' , ').') order by p_order,cat_id desc';
            }else{
                $sqlString = 'SELECT cat_id,cat_name FROM sdb_tuangou_cat WHERE parent_id = '.$id.' order by p_order,cat_id desc';
            }
    ...
    function getFieldById($id, $aField=array('*')) {
        if(is_array($id)){
            $sqlString = "SELECT ".implode(',', $aField)." FROM sdb_tuangou_cat WHERE cat_id in (".implode($id," , ").")";
            return $this->db->select($sqlString);
        }else{
            $sqlString = "SELECT ".implode(',', $aField)." FROM sdb_tuangou_cat WHERE cat_id = ".intval($id);
    ...
    function updateChildCount($id,$cat_id=false){
        ...
        $row = $this->db->selectrow('SELECT count(*) AS num FROM sdb_tuangou_cat WHERE parent_id='.intval($id));
        ...
        $rs = $this->db->exec('SELECT * FROM sdb_tuangou_cat WHERE cat_id='.intval($id));
    ...
    function addNew($data){
        ...
        while($parent_id){
            ...
            $row = $this->db->selectrow('SELECT parent_id, cat_path, p_order FROM sdb_tuangou_cat WHERE cat_id='.intval($parent_id));
        ...
        if($data['cat_id']){
            ...
            $sDefine=$this->db->selectrow('SELECT parent_id FROM sdb_tuangou_cat WHERE cat_id='.intval($data['cat_id']));
            $rs = $this->db->exec('SELECT * FROM sdb_tuangou_cat WHERE cat_id='.$data['cat_id']);
            ...
            if(!$sql || $this->db->exec($sql)){
                ...
                if($sDefine['parent_id']!=$data['parent_id']){
                    ...
                    /*
                    $rs = $this->db->exec('SELECT * FROM sdb_tuangou_cat WHERE parent_id='.$data['cat_id']);
            ...
            unset($data['cat_id']);
            $rs = $this->db->exec('SELECT * FROM sdb_tuangou_cat WHERE 0=1');
    ...
    function updatePath($cat_id,$cat_path){
          $result = $this->db->select('SELECT cat_id,cat_path FROM sdb_tuangou_cat WHERE cat_path like \''.$cat_id.',%\' or parent_id='.intval($cat_id).'');
          foreach($result as $k=>$v){
              if($cat_path==',')
              unset($cat_path);
            $path=$cat_path.substr($v['cat_path'],strpos($v['cat_path'],$cat_id.','),strlen($v['cat_path']));

            $this->db->exec('update sdb_tuangou_cat set cat_path="'.$path.'" where cat_id='.intval($v['cat_id']));
          }
    }

    function toRemove($catid){
/*        $aCats = $this->db->select('SELECT * FROM sdb_tuangou_cat WHERE parent_id = '.intval($catid));
        if(count($aCats) > 0){
        ...
        $row = $this->db->selectrow('SELECT parent_id FROM sdb_tuangou_cat WHERE cat_id='.intval($catid));
        $parent_id = $row['parent_id'];

        $this->db->exec('DELETE FROM sdb_tuangou_cat WHERE cat_id='.intval($catid));
    ...
    function deliveryInfo($aGoodsId,$cat_has_pdt){
        ...
        $sqlString = 'SELECT c.cat_id,t.member_req,t.is_physical FROM sdb_goods g
            LEFT JOIN sdb_tuangou_cat c ON c.cat_id = g.cat_id
    ...
    function getTypeDetail($catid){
        ...
        *        @$catid:商品分类
        */
        $sqlString = 'SELECT c.cat_name,c.cat_id,t.* FROM sdb_tuangou_cat c
            LEFT JOIN sdb_goods_type t ON c.type_id = t.type_id
    ...
    function del($id,&$msg){
        ...
        return $this->db->exec('delete from sdb_tuangou_cat where cat_id='.$id);
    }
    ...
    function getCatidbyAlias($alias){
        ...
        if($alias){
            if(strstr($alias, '->')){
                ...
                $sql = "SELECT cat_id,parent_id,cat_name,cat_path FROM sdb_tuangou_cat WHERE cat_name = '".$cat_name."'";
                    ...
                    foreach($aRows as $k => $row){
                        ...
                        if(count($aId) == count($aCatName)-1){
                            ...
                            foreach($aId as $i => $catid){
                                $sql = "SELECT count(*) AS num FROM sdb_tuangou_cat WHERE cat_name = '".$aCatName[$i]."' AND cat_id =".$catid;
                ...
                $sql = "SELECT cat_id FROM sdb_tuangou_cat WHERE cat_name = '".$alias."'";
    ...
    function getNamePathById($catId){
        $aRet = $this->db->selectrow('SELECT cat_path,cat_name FROM sdb_tuangou_cat WHERE cat_id ='.$catId);
        if($aRet){
            if($aRet['cat_path'] == ',' || $aRet['cat_path'] == '') return $aRet['cat_name'];
            else{
                $catPath = substr($aRet['cat_path'], 0, strlen($aRet['cat_path'])-1);
                $sql = "SELECT cat_id,cat_name FROM sdb_tuangou_cat WHERE cat_id IN('".$catPath."') ORDER BY cat_path";
