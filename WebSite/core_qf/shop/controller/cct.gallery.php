<?php
class cct_gallery extends ctl_gallery{
  function skdis(){
  ...
  function index($view,$cat_id='',$urlFilter=null,$orderBy=0,$tab=null,$page=1,$cat_type=null) {
        ...
        foreach( $this->pagedata['products'] as $k=>$v){
          $this->pagedata['products'][$k]['goods_point']=$objPoint->get_single_point($v['goods_id']);
        }
...
//        $orderBy = (!$orderBy)?3:$orderBy;
        ...
        if($cat_type){
            ...
            $allsearchchild=array();  
            
            if(count($cat_id)>0 && !empty($cat_id[0])){
            $tmpchildnode = $productCat->getCatParentById($cat_id,$view);//获取商品分类
            if(!$tmpchildnode){
           
            //if not childnode,show this parent cat's all childnode-by-sk-2011-10-15          
              $catselfinfo=$productCat->getFieldById($cat_id,array('parent_id'));
              $tmpchildnode=$productCat->getCatParentById($catselfinfo[0]['parent_id'],$view);
              }
            }
            foreach($tmpchildnode as $kc=>$vc){
              $tmpchilds[$vc['cat_id']]['name']=$vc['cat_name'];
              $tmpchilds[$vc['cat_id']]['url']=$this->system->mkUrl('gallery',current($cat['setting']['list_tpl']),array($vc['cat_id'],urlencode($urlFilter2),$orderBy,$tab,$page,$cat_type));//urlfilter2-2012-11-21 22:27:00-to avoid when has pager>1 error
            }
            if(count($tmpchilds)>0){
            $allsearchchild['parenturl']=$this->system->mkUrl('gallery',current($cat['setting']['list_tpl']),array($catselfinfo[0]['parent_id']?$catselfinfo[0]['parent_id']:$cat_id[0],urlencode($urlFilter),$orderBy,$tab,$page,$cat_type));
            $allsearchchild['options']  =$tmpchilds;
            $allsearchchild['name']='分类';
            $allsearchchild['value']=array($cat_id[0]=>0);
            $this->pagedata['searchCatList']['cat_id']=$allsearchchild;

            }
             $this->pagedata['reseturl']=$this->system->mkUrl('gallery',current($cat['setting']['list_tpl']),array($catselfinfo[0]['parent_id']?$catselfinfo[0]['parent_id']:$cat_id[0]));
            //if not childnode,show this parent cat's all childnode-by-sk-2011-10-15
        }
        ...
        if (is_array($cat['brand'])){
            foreach($cat['brand'] as $bk => $bv){
                ...
                //$brand = array('name'=>__('品牌'),'value'=>($filter['brand_id']));//mod by sk
        ...
        /*add brand info to searchSelect by sk-begin*/
        if($selector['brand_id']){
        //print_r($selector['brand_id']);exit;
          $searchBrandList['brand_id']=$selector['brand_id'];
        }
         $this->pagedata['searchBrandList']=$searchBrandList;
        /*add brand info to searchSelect by sk-end*/
        foreach($cat['props'] as $prop_id=>$prop){
            if($prop['search']=='select'){
                ...
                $plugadd=array();
                //循环符合条件的商品计算包含属性的数量begin       
                foreach($goods_relate as $k=>$v){
                            //质地$v[p_1]=1,$v[p_1]=1,2,3,plugadd[0]=,代表纯棉
                            if($v["p_".$prop_id]!=null){
                                ...
                                //}
                            // add foreach by showker,begin p_1=0,1
                           $arrpropids=explode(',',$v["p_".$prop_id]);
                            foreach($arrpropids as $propidone){

                                if($plugadd[$propidone]){
                                    $plugadd[$propidone]=$plugadd[$propidone]+1;
                                }else{
                                    $plugadd[$propidone]=1;
                                }   
                             }
                             //add foreach by showker,end p_1=0,1
                            }
                    $aFilter['goods_id'][] = $v['goods_id'];    //当前的商品结果集
                    $g['goods_id']=$v['goods_id'];/*前台搜索商品规格筛选，所要获取的goods_id*/
                }
                //循环符合条件的商品计算包含属性值的数量begin 
                $navselector=0;
                foreach($prop['options'] as $q=>$e){
                    if($plugadd[$q]){
                        $prop['options'][$q]=$prop['options'][$q]."<span class='num'>(".$plugadd[$q].")</span>";//纯棉(3)
                        if (!$navselector)
                            $navselector=1;
                    }else{
      ...
      //print_r($this->seoTag);exit();
        ...
        $this->pagedata['pricelist0']=$this->system->mkUrl('gallery',$view,array(implode(',',$cat_id),urlencode($p['str']),$orderBy,$tab,($tmp = time()),$cat_type));
       //print_r($this->pagedata['searchselect']);exit;
